...
void main() {
    ...
    for(int x=0;x<=1;x++){
        for(int y=0;y<=1;y++){
            for (int z=0;z<=1;z++){
                vec4 vertexPos = vec4(x*2.0-1.0,y*2.0-1.0,z*2.0-1.0, 1);
                vec4 clipPos = projViewMatrix * transformationMatrix * vertexPos;
    ...
    int lod = int(ceil(log2(max(fullScreenWidth,fullScreenHeight)/2.0)));
